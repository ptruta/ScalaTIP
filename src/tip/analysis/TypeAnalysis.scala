  ...
  def visit(node: AstNode, arg: Unit): Unit = {
    ...
    node match {
      case program: AProgram =>
        // For each function of the program, call visit and generate constraint for the sub-AST
      program.funs.foreach(fun => visit(fun, arg))
      ...
      case as: AAssignStmt =>
        as.left match {
          case id: AIdentifier => unify(id, as.right) // <--- unify
          case dw: ADerefWrite => unify(dw.exp, PointerType(as.right)) // <--- unify
          case dfw: ADirectFieldWrite => unify(AFieldAccess(dfw.id,dfw.field,dfw.loc), as.right) // <--- unify
          case ifw: AIndirectFieldWrite => unify(AFieldAccess(ifw.exp,ifw.field,ifw.loc), PointerType(as.right)) // <--- unify
        }
      case bin: ABinaryOp =>
        bin.operator match {
          case Eqq => unify(bin.left, bin.right); unify(bin, IntType()) // <--- unify
          case _ => unify(bin.left, IntType()); unify(bin.right, IntType()); unify(bin, bin.left); unify(bin, bin.right) // <--- unify
        }
      case un: AUnaryOp =>
        un.operator match {
          case DerefOp => unify(un.subexp, PointerType(un)) // <--- unify
        }
      case alloc: AAlloc => unify(alloc, PointerType(alloc.exp)) // <--- unify
      case ref: AVarRef => unify(ref, PointerType(ref.id)) // <--- unify
      case _: ANull => unify(_, PointerType(FreshVarType())) // <--- unify
